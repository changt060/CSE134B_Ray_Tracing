...
void display() 
{
  ...
  if (numused) {
    ...
    for (int i = 0; i < 4 * numused; i+=4) { 
        GLfloat curLightPosn[] = { lightposn[i], lightposn[i + 1], lightposn[i + 2], lightposn[i + 3] };
        GLfloat curLightPosnTransf[4];
        transformvec(curLightPosn, curLightPosnTransf);
        memcpy(&lightransf[i], curLightPosnTransf, sizeof(curLightPosnTransf));
    }
  ...
  // Applying scaling first, then translation, then modelview to the global 
  // transformation
  ...
  for (int i = 0 ; i < numobjects ; i++) {
    ...
    // Using a new matrix to void mixing the transformations from different objects
    // into the same one
    mat4 netTransForCurObj = transf * obj->transform;
    modelview = netTransForCurObj;
